...
else enemies.push(new Enemy(x,y,radius, 'magenta', velocity ={
  x:Math.cos(angle)*2,
  y:Math.sin(angle)*2,
 },3))
q++
...
function animate() { 
 ctx.fillStyle=  "rgba(10,0,10,0.6)";
  ...
  if(scor>=300 && enemies.length==0){
    ...
    ctx.fillText("Pagube gloante upgradate ! ",canvas.width/2,canvas.height/2+50)
    ctx.fillText("Viteza reincarcare upgradate ! ",canvas.width/2,canvas.height/2+100)
    document.addEventListener('keypress',function(e){
      if(e.key==='Enter'){window.location.href="boss.html"}
