...
let www =new Image();
www.src="enemy2.png";
let shadowbranza =new Image();
shadowbranza.src="branza.png";
let gloante =48;
let scor=0;
let stop =0;
const enemies = [];
...
if(gloante==0){done=0;r.currentTime=0; }
  ...
  setInterval(function(){if(gloante<48 && done==0){r.play();gloante=gloante+4}else{done=1;}},100)
...
if(stop==0){
  if(scor>=100 && caca==1){caca=0;
    ...
    enemies.push(new Enemy(x,y,80, 'black', velocity ={
...
enemies.push(new Enemy(x,y,radius, 'purple', velocity ={
 ...
 else{  enemies.push(new Enemy(this.x,this.y,40, 'magenta', velocity ={
...
{ctx.fillStyle = this.color;
  ...
  ctx.arc(this.x,this.y,this.radius,0,Math.PI*2,false)
  ctx.fill();
  if(this.mhealth==4){ctx.drawImage(ww,this.x-50,this.y-90,100,150)}
  else if(this.mhealth==3){ctx.drawImage(www,this.x-80,this.y-90,160,210)}
  else{ctx.drawImage(shadowbranza,this.x-130,this.y-100,200,200)}
  if(this.mhealth<=4){  ctx.fillStyle="white";
...
{ this.draw();
  ...
  { let hef=this.health*2;
    ctx.fillStyle="red";
    ctx.fillText('ShadowBranzoi',canvas.width/2-100,45)
    ctx.fillRect(canvas.width/2-100,55,hef,45)
    if(this.health%10==0 && uu==0)
    {uu=1;
      let angle = Math.atan2(
      ...
      enemies.push(new Enemy(this.x,this.y,40, 'magenta', velocity ={
      x:Math.cos(angle)*2.5,
      y:Math.sin(angle)*2.5,
     },4))}
...
function animate() { 
 ctx.fillStyle=  "rgba(10,0,10,0.6)";
  ...
  ctx.fillStyle ="black";  
  ctx.fillRect(canvas.width-600,10, 480, 60);
  ctx.fillStyle ="yellow";  
  ctx.fillRect(canvas.width-600,10, 10*gloante, 60);
  ctx.fillStyle ="red";  
  ctx.fillText("GLOANTE:"+gloante,canvas.width-500,50)
  ctx.fillStyle ="cyan";  
  ...
  if(scor>=300 && enemies.length==0){
    ctx.fillStyle= "yellow"
    ctx.fillText("AI CASTIGAT ! CODUL ESTE k !",canvas.width/2,canvas.height/2)
    document.addEventListener('keypress',function(e){
      if(e.key==='Enter'){window.location.href="../adv.html"}
